<?php
/**
 * Widget API: Balkon_Instagram_Feed class
 *
 * @package WordPress
 * @subpackage Widgets
 * @since 1.3.1
 ...
 * @see WP_Widget
 */
class Balkon_Instagram_Feed extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			'classname' => 'balkon_instagram_feed', 
			'description' => __( "Display your instagram images as grid view.",'balkon-add-ons') 
		);
		parent::__construct('balkon-instagram-feed', __('Balkon Instagram','balkon-add-ons'), $widget_ops);
		$this->alt_option_name = 'balkon_instagram_feed';
	}
...
?>
		...
		<p><label for="<?php echo $this->get_field_id( 'clientid' ); ?>"><?php _e( 'Client ID.' ,'balkon-add-ons'); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'access' ); ?>"><?php _e( 'Access Token.' ,'balkon-add-ons'); ?></label>
		...
		<p><?php _e('<a href="http://jelled.com/instagram/access-token" target="_blank">"How to create your Client ID and generate an Instagram Access Token?"</a>','balkon-add-ons');?></p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'get' ); ?>"><?php _e( 'What do you want to display?','balkon-add-ons'); ?></label>
			<select id="<?php echo $this->get_field_id( 'get' ); ?>" name="<?php echo $this->get_field_name( 'get' ); ?>">
				<option value="tagged" <?php selected( $get, 'tagged' ); ?>>
					<?php _e( 'Tagged - Images with a specific tag','balkon-add-ons'); ?>
				</option>
				<option value="user" <?php selected( $get, 'user' ); ?>>
					<?php _e( 'User - Images from a user','balkon-add-ons'); ?>
				</option>
			</select>
		</p>

		<p><label for="<?php echo $this->get_field_id( 'tagname' ); ?>"><?php _e( 'Tag Name (for get Tagged option above).' ,'balkon-add-ons'); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'userid' ); ?>"><?php _e( 'User ID (for get User option above).' ,'balkon-add-ons'); ?></label>
...
There a several free tools available online that will look it up for you.','balkon-add-ons');?></p>

		<p><label for="<?php echo $this->get_field_id( 'limit' ); ?>"><?php _e( 'Number of Images to show. Max of 60' ,'balkon-add-ons'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'columns' ); ?>"><?php _e( 'Columns grid','balkon-add-ons'); ?></label>
			<select id="<?php echo $this->get_field_id( 'columns' ); ?>" name="<?php echo $this->get_field_name( 'columns' ); ?>">
				<option value="1" <?php selected( $columns, '1' ); ?>>
					<?php _e( 'One Column','balkon-add-ons'); ?>
				</option>
				<option value="2" <?php selected( $columns, '2' ); ?>>
					<?php _e( 'Two Columns','balkon-add-ons'); ?>
				</option>
				<option value="3" <?php selected( $columns, '3' ); ?>>
					<?php _e( 'Three Columns','balkon-add-ons'); ?>
				</option>
				<option value="4" <?php selected( $columns, '4' ); ?>>
					<?php _e( 'Four Columns','balkon-add-ons'); ?>
				</option>
				<option value="5" <?php selected( $columns, '5' ); ?>>
					<?php _e( 'Five Columns','balkon-add-ons'); ?>
				</option>
			</select>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'res' ); ?>"><?php _e( 'Image Size','balkon-add-ons'); ?></label>
			<select id="<?php echo $this->get_field_id( 'res' ); ?>" name="<?php echo $this->get_field_name( 'res' ); ?>">
				<option value="thumbnail" <?php selected( $res, 'thumbnail' ); ?>>
					<?php _e( 'Thumbnail - 150x150 px','balkon-add-ons'); ?>
				</option>
				<option value="low_resolution" <?php selected( $res, 'low_resolution' ); ?>>
					<?php _e( 'Low Resolution - 306x306 px','balkon-add-ons'); ?>
				</option>
				<option value="standard_resolution" <?php selected( $res, 'standard_resolution' ); ?>>
					<?php _e( 'Standard Resolution - 612x612 px','balkon-add-ons'); ?>
				</option>
			</select>
		</p>
