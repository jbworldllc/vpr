...
// Add term page
function balkon_portfolio_cat_add_new_meta_field() {
...
add_action('portfolio_cat_add_form_fields', 'balkon_portfolio_cat_add_new_meta_field', 10, 2);

// Edit term page
function balkon_portfolio_cat_edit_meta_field($term) {
    wp_enqueue_media();
    wp_enqueue_script('balkon_tax_meta', CTH_DIR_URL . 'inc/assets/upload_file.js', array('jquery'), null, true);
...
add_action('portfolio_cat_edit_form_fields', 'balkon_portfolio_cat_edit_meta_field', 10, 2);

// Save extra taxonomy fields callback function.
function balkon_save_portfolio_cat_custom_meta($term_id) {
    if (isset($_POST['term_meta'])) {
        ...
        update_option("balkon_taxonomy_portfolio_cat_$t_id", $term_meta);
    }
}
add_action('edited_portfolio_cat', 'balkon_save_portfolio_cat_custom_meta', 10, 2);
add_action('edited_portfolio_cat', 'balkon_save_portfolio_cat_custom_meta', 10, 2);
