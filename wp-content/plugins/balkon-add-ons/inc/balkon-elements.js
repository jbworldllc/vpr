...
function balkon_do_ajax_get_vc_attach_images(images, holder, wrapper){
  jQuery.post({
      dataType: "json",
      url: ajaxurl,
      data: {action: 'balkon_get_vc_attach_images', images: images},
...
window.BalkonImagesView = vc.shortcode_view.extend({
  $wrapper: !1
  , changeShortcodeParams: function (model) {
    var params;
    if (window.BalkonImagesView.__super__.changeShortcodeParams.call(this, model), params = _.extend({}, model.get("params")), this.$wrapper || (this.$wrapper = this.$el.find(".wpb_element_wrapper")), _.isObject(params)){
      ...
      this.$wrapper.find('.wpb_vc_param_value.ajax-vc-img').each(function(){
        ...
        if(param_name !== 'undefined' && "undefined" !== params[param_name] && '' !== params[param_name]){
          balkon_do_ajax_get_vc_attach_images(params[param_name], singleimgHolder, wrapper_jq);
        }
